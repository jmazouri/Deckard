<template>
    <div class="deckEditor">
        ...
        <contextMenu ref="deckOpts">
            ...
            <li class="ctx-item" @click="exportDeck()">Export to Text (.txt)</li>
            <li class="ctx-item" @click="exportDeckCod()">Export to Cockatrice (.cod)</li>
        ...
        <CardGrid class="smaller" :isGrouped="true" :showAllText="false" v-on:addToDeck="addToDeck"
            :cards="currentCards" v-on:removeFromDeck="removeFromDeck" v-if="selectedDeck"></CardGrid>
...
export default class DeckEditor extends Vue
{
    ...
    exportDeck()
    {
        ...
        this.downloadFile(this.selectedDeck.name + ".txt", baseUrl);   
    }

    downloadFile(name: string, content: string)
    {
        let downloadLink = document.createElement("a");
        downloadLink.href = encodeURI(content);
    ...
    exportDeckCod()
    {
        let baseUrl = "data:text/xml,";
        baseUrl += '<cockatrice_deck version="1">';
        baseUrl += `<deckname>${this.selectedDeck.name}</deckname>`;
        baseUrl += '<comments>Generated by Deckard https://github.com/jmazouri/Deckard</comments>';
        baseUrl += '<zone name="main">';

        let cards: any = _.groupBy(this.selectedDeck.cards, function(card: Card) { return card.name });

        for (let cardGrp in cards)
        {
            baseUrl += `<card number="${cards[cardGrp].length}" name="${cardGrp}"/>`;
        }

        baseUrl += '</zone>';
        baseUrl += '</cockatrice_deck>';

        this.downloadFile(this.selectedDeck.name + ".cod", baseUrl);
    }
